...
impl App {
    ...
    fn page_items(&self) -> Vec<(usize, &PackageInfo)> {
        ...
        results.iter().enumerate().skip(start).take(self.page_size).collect()
    }
    ...
    fn selected_absolute_index(&self) -> Option<usize> {
        self.list_state.selected().map(|rel| self.page * self.page_size + rel)
    }
    ...
    fn update_source_options(&mut self) {
        if let Some(pkg) = self.selected_package() {
            let name = &pkg.name;
            let sources: Vec<String> = self.all_results.iter()
    ...
    fn set_results(&mut self, results_by_source: Vec<(String, Vec<PackageInfo>)>) {
        ...
        self.active_tab = 0;
        self.page = 0;
        self.list_state.select(if self.all_results.is_empty() { None } else { Some(0) });
...
fn render(terminal: &mut Terminal<CrosstermBackend<io::Stdout>>, app: &mut App) -> io::Result<()> {
    terminal.draw(|f| {
        let size = f.area();
        // Dark background
        f.render_widget(Block::default().style(Style::default().bg(theme::bg_color())), size);
...
fn render_search_view(f: &mut ratatui::Frame, app: &mut App, area: Rect) {
    ...
    let tab_mode = if app.mode == Mode::Browse && app.source_options.len() > 1 {
        let pkg_name = app.selected_package()
            .map(|p| p.name.as_str())
            .unwrap_or("");
    ...
    let desc_col_max = (chunks[2].width as usize).saturating_sub(5 + 26 + 14 + 12 + 6).max(10);

    let grid_rows: Vec<grid_table::GridRow> = items.iter().map(|(global_idx, pkg)| {
        let installed_marker = if pkg.installed { " ✓" } else { "" };
        let name_raw = format!("{}{}", pkg.name, installed_marker);
        let ver = if pkg.version.is_empty() { "—".to_string() } else { pkg.version.chars().take(12).collect() };
        let src: String = pkg.source.chars().take(10).collect();
        let desc: String = pkg.description.chars().take(desc_col_max).collect();

        grid_table::GridRow {
            cells: vec![
                grid_table::GridCell { text: format!("{}", global_idx + 1), style: theme::number() },
                grid_table::GridCell { text: name_raw, style: if pkg.installed { theme::success() } else { theme::pkg_name() } },
                grid_table::GridCell { text: ver, style: theme::version() },
                grid_table::GridCell { text: src, style: theme::source_tag() },
    ...
    let columns = [
        ...
        grid_table::Column { header: "Name",        width: Constraint::Percentage(25) },
        grid_table::Column { header: "Version",     width: Constraint::Length(14) },
        grid_table::Column { header: "Source",      width: Constraint::Length(12) },
    ...
    let help = match app.mode {
        ...
        Mode::Browse => " ↑↓ navigate • ←→ source/page • tab filter • i install • d remove • / search • q quit",
        _ => " q quit",
    };
...
fn render_confirm_overlay(f: &mut ratatui::Frame, app: &App, area: Rect) {
    ...
    let action = if app.confirm_action == "install" { "Install" } else { "Remove" };
    ...
    let block = Block::default()
        ...
        .title(Span::styled(
            ...
            Style::default().fg(theme::hot_pink()).add_modifier(ratatui::style::Modifier::BOLD),
        ))
    ...
    let text = vec![
        ...
        Line::from(Span::styled(
            format!("  Are you sure you want to {} {}?", action.to_lowercase(), pkgs),
            theme::pkg_name(),
        )),
        Line::from(""),
        Line::from(""),
    ...
    f.render_widget(Paragraph::new(text).block(block).wrap(Wrap { trim: true }), dialog);
}
...
fn render_progress_view(f: &mut ratatui::Frame, app: &App, area: Rect) {
    ...
    let action_text = if app.confirm_action == "install" { "Installing" } else { "Removing" };
    let frame = SPINNER_FRAMES[(app.tick as usize) % SPINNER_FRAMES.len()];
    let frame_style = LGStyle::new().foreground(LGColor::from("63"));
    let spinner_str = frame_style.render(frame);
    let title = Paragraph::new(Text::raw(format!("  {} {} packages...", spinner_str, action_text)))
    ...
    let items: Vec<ListItem> = app.progress_items.iter().enumerate().map(|(i, item)| {
        ...
        let name_style = if item.done {
            ...
            Style::default().fg(Color::Rgb(255, 100, 200)).add_modifier(ratatui::style::Modifier::BOLD) // 212 pink
        };

        let line = Line::from(vec![
            Span::styled(if !item.done { format!("  {} ", icon) } else { icon.to_string() }, icon_style),
    ...
    f.render_widget(
        Paragraph::new(Text::raw(progress_text))
            .block(Block::default()
                .borders(Borders::ALL)
                .border_style(theme::border())
...
fn render_done_view(f: &mut ratatui::Frame, app: &App, area: Rect) {
    ...
    let fail_count = app.progress_items.iter().filter(|i| i.done && !i.success).count();
    let block = Block::default()
        .borders(Borders::ALL)
...
fn handle_browse_key(app: &mut App, key: KeyCode, modifiers: KeyModifiers) -> Option<Action> {
    ...
    match key {
        KeyCode::Up => {
            if let Some(sel) = app.list_state.selected() {
                if sel > 0 {
                    ...
                    app.list_state.select(Some(new_page_items.saturating_sub(1)));
                }
            }
            app.update_source_options();
        }
        ...
        KeyCode::Tab => {
            ...
            app.page = 0;
            app.list_state.select(if app.current_results().is_empty() { None } else { Some(0) });
            app.update_source_options();
        }
        KeyCode::BackTab => {
            app.active_tab = if app.active_tab == 0 { app.tab_titles.len() - 1 } else { app.active_tab - 1 };
            app.page = 0;
            app.list_state.select(if app.current_results().is_empty() { None } else { Some(0) });
            app.update_source_options();
        }
        KeyCode::Char('i') | KeyCode::Enter => {
            if let Some(pkg) = app.selected_package() {
                let _source = if app.source_options.len() > 1 {
                    app.source_options.get(app.source_selected).cloned().unwrap_or_default()
                } else {
                    pkg.source.clone()
                };
...
fn handle_confirm_key(app: &mut App, key: KeyCode) -> Option<Action> {
    let confirm_yes = || {
        ...
        let mut new_app_mode = Mode::Progress;
        let mut progress_items = pkgs.iter().map(|p| ProgressItem {
            name: p.clone(),
            done: false,
            success: false,
        }).collect();
    ...
    match key {
        ...
        KeyCode::Char('n') | KeyCode::Char('N') | KeyCode::Char('c') | KeyCode::Char('C') | KeyCode::Char('b') | KeyCode::Char('B') | KeyCode::Esc => {
            app.mode = Mode::Browse;
            None
        }
        _ => None,
                ...
                if let Some(action) = handle_key(&mut app, key_event.code, key_event.modifiers) {
                    match action {
                        ...
                        Action::FuzzySearch => {
                            ...
                            let mut child_res = Command::new("sk")
                                .arg("--ansi")
                                .stdin(Stdio::piped())
                                .stdout(Stdio::piped())
                                .spawn();
                            ...
                            match child_res {
                                    ...
                                    if output.status.success() {
                                        ...
                                        if !selected_line.is_empty() {
                                            if let Some(idx) = selected_line.rfind(" [") {
                                                ...
                                                let filtered: Vec<PackageInfo> = app.all_results.clone()
                                                    .into_iter()
                                                    .filter(|p| p.name == name)
                                                    .collect();
                                                if !filtered.is_empty() {
                                                    ...
                                                    for p in filtered {
                                                        map.entry(p.source.clone()).or_insert_with(Vec::new).push(p);
                                                    }
                                                    let mut new_results: Vec<(String, Vec<PackageInfo>)> = map.into_iter().collect();
                                ...
                                Err(_) => {
                                    ...
                                    execute!(terminal.backend_mut(), EnterAlternateScreen, cursor::Hide)?;
                                    terminal.clear()?;
                                    app.status_message = "Error: Skim (sk) fuzzy finder is not installed.".to_string();
                                }
                            }
                        }
                    }
...
pub async fn run_main_tui(
    ...
    let search_managers: Vec<Box<dyn PackageManager>> = if let Some(requested) = requested_manager {
        managers.into_iter().filter(|m| m.name() == requested).collect()
    } else {
        managers
    };
                    ...
                    if let Event::Key(key) = event::read()? {
                        if let Some(action) = menu_state.handle_key(key.code) {
                            match action {
                                ...
                                menu::MenuAction::FileManager => current_mode = AppMode::FileManager,
...
pub async fn run_progress_tui(
    ...
    app.progress_items = items.iter().map(|p| ProgressItem {
        name: p.clone(),
        done: false,
        success: false,
    }).collect();
    ...
    loop {
        if event::poll(Duration::from_millis(100))? {
            if let Event::Key(key_event) = event::read()? {
                if matches!(key_event.code, KeyCode::Char('q') | KeyCode::Esc | KeyCode::Enter) {
                    break;
                }
            }
        }
...
async fn search_all(
    ...
    let mut results: std::collections::HashMap<String, Vec<PackageInfo>> = std::collections::HashMap::new();
    ...
    for pm in managers {
        ...
        for term in &terms {
            if let Ok(pkgs) = pm.search_captured(term).await {
                if !pkgs.is_empty() {
                    results.entry(pm.name().to_string()).or_default().extend(pkgs);
                }
            }
        }
    }
